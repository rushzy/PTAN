...
class BertSelfAttention(nn.Module):
    def __init__(self, seq_type, config):
        ...
        # self.edge_fc = nn.Linear(config.hidden_size, config.num_rules)
        ...
        # self.edge_fc_other = nn.Linear(config.hidden_size, config.num_rules)

        # self.edge_pred = nn.Linear(config.num_rules, 1)

        # self.fuzzy_mean = nn.Parameter(torch.randn((1, 1, 1, config.num_rules)))
        # self.fuzzy_var = nn.Parameter(torch.randn(1, 1, 1, config.num_rules))

        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)

        # self.lamda = config.graph_ema_lamda
    ...
    def forward(self, hidden_states, hidden_states_other, attention_mask, output_attention_probs=False):
        ...
        # other_query_layer = self.query_other(hidden_states_other)
        ...
        # attention_probs = self.dropout(attention_probs) * edge.unsqueeze(dim=1)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_other, attention_mask, output_attention_probs=False):
        ...
        if output_attention_probs:
            ...
            attention_output_other = attention_output_other
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_other, attention_mask, output_all_encoded_layers=False, output_attention_probs=False):
        ...
        for layer_module in self.layer:
            layer_out = layer_module(hidden_states, hidden_states, attention_mask, output_attention_probs=output_attention_probs)
            if output_attention_probs:
                hidden_states, hidden_states, attention_probs, attention_probs_other = layer_out

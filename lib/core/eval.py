...
def display_results(eval_result, miou, title=None, return_target = None):
    tious = [float(i) for i in config.TEST.TIOU.split(',')] if isinstance(config.TEST.TIOU,str) else [config.TEST.TIOU]
    recalls = [int(i) for i in config.TEST.RECALL.split(',')] if isinstance(config.TEST.RECALL,str) else [config.TEST.RECALL]
    ...
    if return_target is None or return_target == "mIoU" :
        return table.table, miou
    elif isinstance(return_target, list):
        return table.table, eval_result[tious.index(return_target[1])][recalls.index(return_target[0])]
    else:
        raise ValueError("wrong return_target {}, expected [R@K, IoU=M]".format(return_target))
...
if __name__ == '__main__':
    ...
    train_data = json.load(open('/data/home2/train_data.json', 'r'))
    val_data = json.load(open('/data/home2/val_data.json', 'r'))
